...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, element) => {
    return acc + element.charCodeAt(0);
  }, 0);
}
...
function encodeToRot13(str) {
  const MIN_VALUE_LOWER_CHAR = 97;
  const MAX_VALUE_LOWER_CHAR = 122;
  const MIN_VALUE_UPPER_CHAR = 65;
  const MAX_VALUE_UPPER_CHAR = 90;
  const VALUE_LOWER_N = 110;
  const VALUE_UPPER_N = 78;

  const loverAlphabet = {};
  const upperAlphabet = {};

  for (let i = MIN_VALUE_LOWER_CHAR; i <= MAX_VALUE_LOWER_CHAR; i += 1) {
    if (i < VALUE_LOWER_N) {
      loverAlphabet[String.fromCharCode(i)] = String.fromCharCode(i + 13);
    } else {
      loverAlphabet[String.fromCharCode(i)] = String.fromCharCode(i - 13);
    }
  }

  for (let i = MIN_VALUE_UPPER_CHAR; i <= MAX_VALUE_UPPER_CHAR; i += 1) {
    if (i < VALUE_UPPER_N) {
      upperAlphabet[String.fromCharCode(i)] = String.fromCharCode(i + 13);
    } else {
      upperAlphabet[String.fromCharCode(i)] = String.fromCharCode(i - 13);
    }
  }
  ...
  function changeChar(char) {
    if (
      char.charCodeAt(0) > MAX_VALUE_LOWER_CHAR ||
      char.charCodeAt(0) < MIN_VALUE_UPPER_CHAR ||
      (char.charCodeAt(0) > MAX_VALUE_UPPER_CHAR &&
        char.charCodeAt(0) < MIN_VALUE_LOWER_CHAR)
    ) {
      return char;
    }
    if (char === char.toLowerCase()) {
      return loverAlphabet[char];
    }

    return upperAlphabet[char];
  }

  return str
    .split('')
    .map((el) => changeChar(el))
    .join('');
}
